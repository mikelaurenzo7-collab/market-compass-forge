      ...
      <div className="flex gap-1 p-1 rounded-lg bg-muted/30 border border-border w-fit">
        <button
          ...
          className={`px-4 py-1.5 rounded-md text-xs font-medium transition-all ${
            view === "flows" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"
          }`}
        >
          Capital Flows
        </button>
        <button
          ...
          className={`px-4 py-1.5 rounded-md text-xs font-medium transition-all ${
            view === "rotation" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"
          }`}
        >
          Rotation Map
        </button>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {s.dealCount} deals · {formatCurrency(s.totalCapital)}
                    </p>
                  </div>
          ...
          <div className="lg:col-span-2 rounded-lg border border-border bg-card p-4">
            ...
            <div className="text-sm text-muted-foreground space-y-2">
              {gaining.length > 0 && declining.length > 0 ? (
                <>
                  <p>
                    Capital is rotating <span className="text-primary font-medium">into {gaining.slice(0, 3).map(g => g.sector).join(", ")}</span>
                    {" "}and <span className="text-destructive font-medium">out of {declining.slice(0, 3).map(d => d.sector).join(", ")}</span>.
                  </p>
                  <p>
                    The strongest inflow is <span className="text-foreground font-medium">{gaining[0]?.sector}</span> at{" "}
                    <span className="text-primary font-mono">+{gaining[0]?.trend.toFixed(0)}%</span> capital growth, with AI signals pointing{" "}
