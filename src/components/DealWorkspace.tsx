...
import { X, MessageSquare, CheckCircle2, Vote, ThumbsUp, ThumbsDown, Minus, FileText, Loader2, Send } from "lucide-react";
...
export default function DealWorkspace({ dealId, companyId, companyName, onClose }: DealWorkspaceProps) {
  ...
  const [activeTab, setActiveTab] = useState<"notes" | "tasks" | "vote">("notes");
  ...
  // Fetch team activity for this deal
  const { data: activity } = useQuery({
    queryKey: ["deal-activity", companyId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("team_activity")
        .select("*")
  ...
  const tabs = [
    { id: "notes" as const, label: "Notes", icon: <MessageSquare className="h-3.5 w-3.5" /> },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-background/80 backdrop-blur-sm">
      <div className="w-full sm:max-w-2xl max-h-[90vh] sm:max-h-[85vh] rounded-t-lg sm:rounded-lg border border-border bg-card shadow-2xl flex flex-col">
        ...
        {/* Tabs */}
        <div className="px-4 border-b border-border flex gap-1 shrink-0">
          {tabs.map(tab => (
            <button
              key={tab.id}
                    ...
                    {activity.map(a => (
                      <div key={a.id} className="flex items-start gap-2 text-xs">
                        ...
                        <div className="flex-1">
                          ...
                          <span className="text-muted-foreground/60 ml-2">{formatDistanceToNow(new Date(a.created_at), { addSuffix: true })}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full h-9 px-3 pr-10 rounded-md border border-border bg-secondary text-sm text-foreground placeholder:text-muted-foreground"
                  />
                </div>
              </div>
