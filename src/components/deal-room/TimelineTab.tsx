  ...
  return (
    <div className="max-w-2xl space-y-1">
      {decisions.map((d: any, i: number) => {
        const isFieldMemo = d.decision_type === "field_memo";
        return (
          <div key={d.id} className="relative flex gap-3">
            <div className="flex flex-col items-center">
              <div className={`h-7 w-7 rounded-full border-2 bg-card flex items-center justify-center shrink-0 ${
                isFieldMemo ? "border-accent" : d.decision_type === "stage_change" ? "border-primary" : "border-border"
              }`}>
                {isFieldMemo ? <Mic className="h-3 w-3 text-accent-foreground" /> : <Clock className="h-3 w-3 text-muted-foreground" />}
              </div>
              {i < decisions.length - 1 && <div className="w-px flex-1 bg-border/50" />}
            </div>
            <div className="pb-4 flex-1">
              <div className={`rounded-lg border p-3 ${isFieldMemo ? "border-accent/30 bg-accent/5" : "border-border bg-card"}`}>
                <div className="flex items-center gap-2">
                  ...
                  {isFieldMemo && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-accent/20 text-accent-foreground font-medium flex items-center gap-0.5">
                      <MapPin className="h-2.5 w-2.5" /> On-Site
                    </span>
                  )}
                </div>
                {d.rationale && (
                  <p className="text-xs text-muted-foreground mt-1 whitespace-pre-wrap">{d.rationale}</p>
                )}
