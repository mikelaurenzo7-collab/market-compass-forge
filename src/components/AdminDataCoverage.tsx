...
import { Shield, Clock, Database, Loader2 } from "lucide-react";
...
async function fetchDatasetStats(table: string): Promise<Omit<DatasetStats, "label">> {
  const { data, error } = await supabase
    .from(table as any)
    .select("source_type, verification_status, fetched_at");
  if (error) throw error;

  const records = (data ?? []) as any[];
  const bySource: Record<string, number> = {};
  const byVerification: Record<string, number> = {};
  let fresh7d = 0, fresh30d = 0, fresh90d = 0;
  const now = Date.now();
  ...
  return { table, total: records.length, bySource, byVerification, fresh7d, fresh30d, fresh90d };
}
      ...
      const results = await Promise.all(
        DATASETS.map(async (ds) => {
          try {
            ...
            return { table: ds.table, label: ds.label, total: 0, bySource: {}, byVerification: {}, fresh7d: 0, fresh30d: 0, fresh90d: 0 };
          }
        })
      );
      return results;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <Card>
          <CardContent className="pt-5 text-center">
            <Shield className="h-6 w-6 text-success mx-auto mb-1.5" />
        ...
        <Card>
          <CardContent className="pt-5 text-center">
            <Clock className="h-6 w-6 text-warning mx-auto mb-1.5" />
        ...
        <Card>
          <CardContent className="pt-5 text-center">
            <Database className="h-6 w-6 text-muted-foreground mx-auto mb-1.5" />
            <p className="text-2xl font-black font-mono text-foreground">{DATASETS.length}</p>
            <p className="text-[10px] text-muted-foreground mt-0.5">Datasets</p>
          </CardContent>
        </Card>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              ...
              <th className="text-left py-2 px-3 text-[11px] uppercase tracking-wider text-muted-foreground font-medium">Source Breakdown</th>
              ...
              <th className="text-right py-2 px-3 text-[11px] uppercase tracking-wider text-muted-foreground font-medium">90d</th>
            </tr>
          </thead>
          <tbody>
            {(stats ?? []).map((ds) => {
              ...
              return (
                <tr key={ds.table} className="border-b border-border/50 hover:bg-secondary/30">
                  ...
                  <td className="py-2.5 px-3">
                    <div className="flex flex-wrap gap-1">
                      {Object.entries(ds.bySource).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([src, count]) => (
                        <Badge key={src} variant="outline" className="text-[9px] font-mono">
                          {src}: {count}
                        </Badge>
                      ))}
                    </div>
                  ...
                  <td className="py-2.5 px-3 text-right font-mono text-xs">
                    {ds.total > 0 ? Math.round((ds.fresh90d / ds.total) * 100) : 0}%
                  </td>
                </tr>
              );
            })}
