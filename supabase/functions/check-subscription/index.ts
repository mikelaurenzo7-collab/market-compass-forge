...
serve(async (req) => {
  ...
  try {
    ...
    if (customers.data.length === 0) {
      ...
      return new Response(JSON.stringify({ subscribed: false, plan: "essential" }), {
    ...
    if (hasActive) {
      ...
      // Sync to subscription_tiers table
      await supabaseClient.from("subscription_tiers").upsert(
        { user_id: user.id, tier: plan },
        { onConflict: "user_id" }
      );
    }

    // Get upcoming invoice for renewal info
